...
the spine-first protocol §"The per-paper metadata bundle":
...
Module discovery: per-paper `<key>.yaml` modules + any `**/ONTOLOGY.yaml`.
The `paper_key` declared INSIDE each module is authoritative (not the path).

Run:  uv run python build_ontology.py            # dry-run
      uv run python build_ontology.py --apply
      uv run python build_ontology.py --strict   # warnings -> errors
"""
...
def discover_modules() -> list[Path]:
    paths: set[Path] = set()
    paths.update((REPO / "ontology").glob("*.yaml"))
    paths.update(REPO.glob("**/ONTOLOGY.yaml"))
    # Per-paper modules are lowercase paper-key filenames (e.g. 2026a.yaml,
    # foam-bridge-note.yaml). UPPERCASE-stem YAMLs in the ontology directory are
...
def main() -> None:
    ...
    con.close()
    print(
        "Regenerate the .sql mirror: uv run python sync_substrate.py --sql-only"
    )
